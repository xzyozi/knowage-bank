...
def update_article_date(filepath: str, date: datetime):
    ...
    try:
        ...
        formatted_display = f"作成日: {date.year}年{date.month}月{date.day}日"
        
        # 正規表現による time タグの置換
        # パターン: <p class="article-created"><time datetime="...">作成日: ...</time></p>
        pattern = r'(<p class="article-created"><time datetime=")[^"]*(">[^<]*</time></p>)'
        replacement = f'\\1{formatted_datetime}\\2'
        new_content, count = re.subn(pattern, lambda m: f'{m.group(1)}{formatted_datetime}">作成日: {date.year}年{date.month}月{date.day}日</time></p>', content)
        
        # 直書きの time 要素用（template用）
        if count == 0:
            pattern_simple = r'(<time datetime=")[^"]*(">[^<]*</time>)'
            new_content, count = re.subn(pattern_simple, f'\\1{formatted_datetime}">作成日: {date.year}年{date.month}月{date.day}日</time>', content)
...
def main():
    ...
    recent_html = "\n".join(recent_html_parts) if recent_html_parts else '            <p class="empty-note">新着記事はまだありません。</p>'
    ...
    for domain in domains:
        ...
        for cl_id in sorted_clusters:
            # クラスタ内の記事を日付降順で取得
            cluster_arts = sorted([a for a in articles if a["cluster_id"] == cl_id], key=lambda x: x["date"], reverse=True)
            if not cluster_arts:
                continue
        ...
        domain_html_dict[domain] = "\n".join(domain_html_parts) if domain_html_parts else '            <p class="empty-note">記事はまだありません。</p>'
