...
class PersonalKnowledgeService:
    ...
    def __init__(
        ...
        Args:
            ...
            github_client: GitHub クライアント。None の場合は環境変数から初期化。
        """
        self.daos = daos or [
        ...
        self.github_client = github_client or GitHubIssueClient()
    ...
    def run_pipeline(
        ...
        Args:
            dry_run: True の場合は GitHub への起票・コメント追記を行わず判定のみ返す。
        ...
        open_issues = (
            ...
            else self.github_client.get_open_issues()
        )
        ...
        for session in sessions:
            ...
            if not dry_run:
                if decision.action == "add_comment" and decision.target_issue_number:
                    success = self.github_client.add_comment(
                        decision.target_issue_number, decision.body
                    )
                    if success:
                        commented_count += 1
                elif decision.action == "create_issue":
                    new_number = self.github_client.create_issue(decision.title, decision.body)
                    if new_number:
                        created_count += 1
