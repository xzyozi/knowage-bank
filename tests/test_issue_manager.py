...
def test_load_db_invalid_json(temp_db_path):
...
def test_get_next_unprocessed_issue(temp_db_path):
...
def test_update_issue_status(temp_db_path):
    """IM-DB-05/06: ステータスの更新動作（正常系・異常系）"""
    manager = IssueManager(db_path=temp_db_path)
    test_data = {
        "last_sync_at": None,
        "issues": {
            "10": {"number": 10, "title": "Issue 10", "status": "unprocessed", "processed_at": None, "article_file": None}
        }
    }
    manager._save_db(test_data)
...
@patch("httpx.Client")
def test_sync_issues_initial(mock_client_class, temp_db_path):
...
@patch("httpx.Client")
def test_sync_issues_since(mock_client_class, temp_db_path):
    ...
    test_data = {
        ...
        "issues": {
            "1": {"number": 1, "title": "Old Title", "state": "open", "status": "unprocessed"}
        }
    }
    manager._save_db(test_data)
    ...
    # 差分更新用APIデータ
    mock_api_data = [
        {"number": 1, "title": "Updated Title", "state": "closed", "body": "Fixed"}
    ]
    mock_response = MagicMock()
    mock_response.json.return_value = mock_api_data
    mock_response.headers = {}
...
@patch("httpx.Client")
def test_sync_issues_pagination(mock_client_class, temp_db_path):
