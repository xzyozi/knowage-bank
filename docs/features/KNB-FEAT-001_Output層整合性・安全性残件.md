...
| ID      | 優先度 | 状態               | 機能                   | 完了条件                                                                                                                                                                                                            |
| :------ | :----: | :----------------- | :--------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FEAT-03 |   P1   | 実装済み・検証待ち | Markdown検証の拡充     | 必須frontmatter、未対応構文、引用と参考文献の整合を明示的に拒否または許可し、相対リンクの許可範囲を決定する。ローカルの検証依存が利用可能になった後、単体テスト・lint・型検査で確認する。                           |
| FEAT-04 |   P1   | 実装中・検証待ち   | Output同期の統合テスト | 原本・HTML・index・状態の成功／失敗経路をモックで検証し、`mcp`がない環境でもテストを収集できる。IT-OUT-01〜06を実装済みとし、IT-OUT-07は既存単体テスト、IT-OUT-08は未実装とする。ローカルpytestでの検証待ちである。 |
