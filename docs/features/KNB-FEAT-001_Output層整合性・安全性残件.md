...
| FEAT-04 |   P1   | 実装済み・検証待ち | Output同期の統合テスト | 原本・HTML・index・状態の成功／失敗経路をモックで検証し、`mcp`がない環境でもテストを収集できる。IT-OUT-01〜06を実装済みとし、IT-OUT-07は既存単体テストで保証する。IT-OUT-08は再試行承認ワークフローの将来機能としてFEAT-04の対象外とする。ローカルpytestでの検証待ちである。 |
