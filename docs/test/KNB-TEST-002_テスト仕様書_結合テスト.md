...
IT-OUT-08は、既存のOutput同期に存在しない再試行承認用の公開seam/APIを新設する必要があるため、FEAT-04の対象外とする。`failed` Issueは現行どおり自動再試行せず、将来の再試行承認ワークフロー機能として管理する。

## 3. テスト実行
FEAT-04の統合テストは`pytest --run-integration tests/test_output_sync_integration.py`で実行する。テストは`tmp_path`配下の状態JSON、原本、HTML、indexを使用する。通常のpytest実行では`integration`マーカーによりスキップする。IT-OUT-07は既存単体テストで保証し、IT-OUT-08は本コマンドの対象外とする。
