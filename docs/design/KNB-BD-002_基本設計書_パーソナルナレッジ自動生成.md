...
| 版数           | Rev.1.10（Geminiモデル解決・フォールバック基盤の実装反映） |
| 改訂日         | 2026-08-28                                                 |
| 作成日         | 2026-08-23                                                 |
| 作成者         | 開発チーム                                                 |
...
3. **意図判定フィルタとベクトル埋め込みクラスタリング (Intent Filtering & Semantic Clustering)**:
   - クラウド連携オプションとして、Google Gemini APIによる「ナレッジ意図判定（True/False）」と `models/text-embedding-004` & コサイン類似度による高精度な意味的セッション分離をサポートする。
   - `IntentFilter` は `PersonalKnowledgeService` に未指定またはインスタンス指定で有効となり、明示的に `False` を指定した場合だけ無効となる。`SemanticClusterer` はインスタンスを明示指定した場合のみ有効となる。
   - 公式コレクタCLIではGemini連携を既定で有効にし、`--no-gemini` 指定時だけ両機能を無効化してルールベース処理へ切り替える。
...
flowchart TD
    ...
    subgraph ModelInfra ["モデル解決・運用基盤 (Infrastructure)"]
        ModelResolver["ModelResolver\n(API一覧検出・TTLキャッシュ・用途別候補選定)"]
        ModelCache[("モデル候補キャッシュ")]
    ...
    Deduplicator --> Filter
    ModelResolver -- "生成候補" --> Filter
    ModelResolver -- "Embedding候補" --> Analyzer
    Filter --> Analyzer
    Analyzer --> Router
    Router --> BaseClient
...
src/scripts/
└── run-personal-knowledge-collector.py   # CLI実行エントリーポイント（§5.2参照。既定でGemini連携を有効化し、--no-gemini で無効化）
...
> **Gemini連携の有効化契約**:
> - `PersonalKnowledgeService` を直接利用する場合、`intent_filter` は未指定または `IntentFilter` のインスタンス指定で有効となる。ルールベース処理だけを使う場合は `False` を明示指定する。
> - `semantic_clusterer` は `SemanticClusterer` のインスタンスを明示指定した場合のみ有効となる。
> - 公式コレクタCLIはGemini連携を既定で有効化し、`--no-gemini` 指定時に `intent_filter=False` と `semantic_clusterer=None` を渡してルールベース処理へ切り替える。
>
...
| **意図判定フィルタ**                | `IntentFilter`                   | 技術キーワード辞書にない単一語を除外し、残るクエリをGemini APIでバッチ二値判定する。API未設定・障害時は処理継続のため通過（`True`）させる。                                           |
| **セッション分割 / クラスタリング** | `Analyzer` / `SemanticClusterer` | **ルールベース**: 30分以内の連続検索をグループ化し単発破棄。<br>**Embeddingベース**: `models/text-embedding-004` とコサイン類似度 ($\ge 0.70$) で意味的なセッションにクラスタリング。 |
...
| `ModelResolution` | Infrastructure           | 用途別に解決された実利用モデル、候補ソース、フォールバック履歴   |
...
| `--no-gemini`   |  任意  | 無効                           | `IntentFilter`（意図判定フィルタ）および `SemanticClusterer`（Embeddingクラスタリング）を無効化する。未指定時は両機能を有効化し、`GEMINI_API_KEY` が未設定またはAPI障害時は意図判定対象を通過させて処理を継続する。 |

### 5.3 安全回路・システム境界方針
- **サイレントエラー契約**: DAO層でのブラウザDB読み込みエラー（ロック中など）はログ出力のみで処理を中断せず安全にスキップする。
- **キュー安全性**: ネットワーク断や障害発生時は次回サイクルへ安全に繰り延べ、セッション喪失や重複起票を防ぐ。

### 5.4 Geminiモデル解決・フォールバック基盤
Geminiのモデル名をアプリケーションコードへ固定せず、`ModelResolver` をインフラコンポーネントとして設ける。意図判定（`generateContent`）と意味的クラスタリング（`embedContent`）は必要なAPI機能が異なるため、用途別に独立した候補リストを解決する。

- **検出契機**: パイプライン開始時、モデル一覧キャッシュのTTL切れ時（既定24時間）、選択済みモデルの404時、または明示的なモデル一覧更新要求時に、Gemini APIのモデル一覧を取得する。
- **選定方針**: 利用可能一覧から用途に対応するモデルだけを抽出し、許可リスト・プレビュー版の可否・安定版優先・数値バージョン降順のポリシーで並べる。APIが提供しないリリース日時や文字列ソートだけを「最新」の根拠にしてはならない。
- **フォールバック**: 404は候補キャッシュを無効化して次候補を選ぶ。429と一時的なネットワーク／5xx障害は限定回数リトライ後に次候補へ進む。401・403・400は認証・権限・リクエスト不備であり、候補切替で隠蔽せず失敗として報告する。
- **可用性と追跡可能性**: 一覧取得に失敗した場合は設定ファイルの緊急候補へフォールバックする。各実行では、用途別の実利用モデル、候補の取得元、フォールバック回数・理由を `ModelResolution` と実行ログへ記録し、必要に応じて実行結果JSONまたはIssueメタデータへ出力する。

> **実装済み**: `ModelResolver` は、既存の固定候補モデル方式を置き換え、動的検出・用途別候補解決・TTLキャッシュ・障害別フォールバックを提供する。
...
| Rev.1.8  | 2026-08-28 | 開発チーム | 実装整合: Gemini連携の既定有効化、`--no-gemini`による無効化、動的一単語判定、およびAPI未設定・障害時に処理を継続する通過フォールバックを反映                |
| Rev.1.9  | 2026-08-28 | 開発チーム | 設計追加: Gemini APIの動的モデル検出、用途別解決、TTLキャッシュ、障害別フォールバック、および実利用モデルの記録基盤を追加                                   |
| Rev.1.10 | 2026-08-28 | 開発チーム | 実装反映: `ModelResolver`、用途別モデル選別、TTLキャッシュ、障害別フォールバック、CLIの実利用モデル出力、設定スキーマを追加                                 |
