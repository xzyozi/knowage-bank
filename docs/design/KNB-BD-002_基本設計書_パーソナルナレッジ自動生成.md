...
| 版数           | Rev.1.4 (未実装機能の明記・変更管理方針章とCLI仕様の追加) |
| 改訂日         | 2026-08-25                                                |
| 作成日         | 2026-08-23                                                |
| 作成者         | 開発チーム                                                |
...
3. **意図判定フィルタとベクトル埋め込みクラスタリング (Intent Filtering & Semantic Clustering)**:
   ...
   - > **実装状況 (2026-08-25時点): 未実装（設計のみ）**。現行の `src/personal_knowledge/` にはブラックリスト判定のみが実装されており、Gemini APIによる意図判定・Embeddingクラスタリングのコードは存在しない。本節は将来実装の設計方針として記述する。
...
| **意図判定フィルタ**                | `IntentFilter`                   | ブラックリストキーワード（天気、ナビ、娯楽等）を除外し、必要に応じて Gemini API (`gemini-1.5-flash`) で技術・ナレッジ調査目的（True）か日常消費目的（False）かを二値判定する。<br>**実装状況: Gemini API判定部分は未実装（設計のみ）**                    |
| **セッション分割 / クラスタリング** | `Analyzer` / `SemanticClusterer` | **ルールベース**: 30分以内の連続検索をグループ化し単発破棄。（実装済み）<br>**Embeddingベース**: `text-embedding-004` (768次元) とコサイン類似度 ($\ge 0.70$) で意味的なセッションにクラスタリング。<br>**実装状況: Embeddingベースは未実装（設計のみ）** |
...
> **実装状況 (2026-08-25時点): 未実装（設計のみ）**。`BaseIssueClient.close_issue` メソッド自体は実装済みだが、本節が定める「12時間無更新検出 → LLM呼び出し → Markdown生成 → Close」のOutput層パイプラインを呼び出す実装コードは `src/personal_knowledge/` に存在しない。
...
| Rev.1.4 | 2026-08-25 | 開発チーム | レビュー対応: Gemini API意図判定/Embeddingクラスタリング/Output層が未実装(設計のみ)である旨を明記。変更管理方針章(§6)とCLIエントリーポイント仕様(§5.2)を追加 |
