...
## 3. 実装アーキテクチャ（Jinja2 + Markdown構造化出力）

記事本文の外部入力とLLM出力はMarkdownのみを正規入力とする。`generate-article.py`、`generate-article-from-mcp.py`、`sync-github-issues.py`はMarkdownを生成・検証し、`ArticleBuilder`へ`markdown_text`として渡す（詳細はKNB-DD-002を参照）。

```mermaid
flowchart TD
    User["ユーザー / Issue (質問テーマ)"] --> MCP["Deep Research (MCP) / リサーチ実行"]
    MCP --> MidData["中間成果物 (Markdown リサーチ結果)"]
    MidData --> Markdown["LLM Markdown記事生成"]
    Markdown --> Stage05["Stage 0.5: クレンジング & 参考文献フッター分離\n(markdown_cleaner.py)"]
    Stage05 --> Stage3["Stage 3: Markdown→HTML変換 & 引用リナンバリング\n(markdown_parser.py + citation_processor.py)"]
...
1. **`src/scripts/generate-article.py` / `generate-article-from-mcp.py` / `sync-github-issues.py`**
   - ローカルLLM（Ollama上の `gemma4-12b` や `qwen2.5` 等）にMarkdown記事を要求し、保存前に検証する。
   - 手動CLIの生応答は`logs/llm_output.log`へ保存する。
2. **`src/app/article_builder.py`**
   - `markdown_text`を必須入力とし、`parse_reference_footer`（Stage 0.5）→ `FlexibleMarkdownParser`（Stage 3a）→ `apply_citations`（Stage 3b）を経由して`site.css`適合HTMLを生成する。
   - カテゴリ表記ゆれを`category_config.json`の正式名称へ正規化する。
