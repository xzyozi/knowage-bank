...
author: "開発チーム"
purpose: "public/articles/ 配下に配置する各記事 HTML の構造、必須要素、命名規則、マークアップパターンをプログラム仕様書の粒度で定義するため"
related_documents:
  - "KNB-BD-001_基本設計書.md"
...
# 詳細設計書（記事 HTML 仕様）
**記事 HTML マークアップ・セクション構造・要素仕様**

| 項目 | 内容 |
| :--- | :--- |
| 文書番号 | KNB-DD-002 |
| ドキュメント名 | 詳細設計書（記事 HTML 仕様） |
| 版数 | Rev.1.2 |
| 改訂日 | 2026-08-15 |
| 作成日 | 2026-06-14 |
| 作成者 | 開発チーム |
...
| 項目             | 値                                                        |
| ---------------- | --------------------------------------------------------- |
| 配置ディレクトリ | `public/articles/`（サブディレクトリ禁止）                |
...
> **補記**: 自由形式 Markdown パイプライン（§16）経由で生成された記事では、本セクションの `<ul>` 方式に代わり §16.2 の `ref_list`（`<ol>` + アンカーリンク方式）が優先される。テンプレート実装上、`ref_list` が存在する場合は `<ol>` で「参考文献」として描画され、存在しない場合のみ本節の `<ul>` 形式にフォールバックする。
...
| Rev.1.2 | 2026-08-15 | 開発チーム | ドキュメント間整合性レビュー反映：§10にref_list優先の補記追加、KNB-DD-003およびKNB-DD-007への相互参照を追加 |
