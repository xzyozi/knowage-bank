...
| 版数           | Rev.1.6（Gemini有効化方式・意図判定障害時挙動の実装反映）                 |
| 改訂日         | 2026-08-28                                                                |
| 作成日         | 2026-08-23                                                                |
| 作成者         | 開発チーム                                                                |
...
* **手順**:
  ...
  2. **LLM意図判定 (`judge_batch_with_llm`)**: 判定対象を最大50件ずつまとめ、`google-genai` SDK (`google.genai.Client`) でGeminiへ送信する。既定の先頭候補モデルは `gemini-3.6-flash` であり、429時は最大3回リトライし、404時は次の候補モデルを試行する。
...
| **Domain層**      | Gemini API 意図判定/埋め込みエラー     | `IntentFilter.judge_batch_with_llm` / `SemanticClusterer._embed_text` 内で例外を捕捉し警告ログを出力。可用性優先で、意図判定は `True`（通過）、埋め込みは空リスト（ゼロベクトル相当）を返却 |
